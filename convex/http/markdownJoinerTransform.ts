  ...
  return new TransformStream<any, any>({
    transform(chunk, controller) {
      if (chunk.type === 'text-delta') {
        const deltaText: string = (chunk as any).text ?? '';
        const processedText = joiner.processText(deltaText);
        if (processedText) controller.enqueue({ ...chunk, text: processedText });
      } else {
    ...
    flush(controller) {
      ...
      if (remaining) controller.enqueue({ type: 'text-delta', id: 'markdown-joiner', text: remaining } as any);
    },
  });
};
